...
function [success, cardInfo] = spcMSetupTrigExternalLevel (cardInfo, extMode, level0, level1, trigTerm, acCoupling, singleSrc, extLine)
    
    error = 0;

    % ----- setup the external trigger mode -----
    % 40510 = SPC_TRIG_EXT0_MODE, i.e pos, neg, etc...
    error = error + spcm_dwSetParam_i32 (cardInfo.hDrv, 40510 + extLine, extMode);     
    % 40110 = SPC_TRIG_TERM0
    error = error + spcm_dwSetParam_i32 (cardInfo.hDrv, 40110 + extLine, trigTerm);    
    % 40120 = SPC_TRIG_EXT0_ACDC
    error = error + spcm_dwSetParam_i32 (cardInfo.hDrv, 40120 + extLine, acCoupling);  
